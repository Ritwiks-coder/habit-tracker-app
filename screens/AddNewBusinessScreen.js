...
// 1. We added "Custom" to the bottom of the list
const CATEGORIES = [
  'Cafe / Coffee Shop',
  'Restaurant / Bar',
  'Cloud Kitchen',
...
export default function AddNewBusinessScreen({ navigation }) {
  const [shopName, setShopName] = useState('');
  const [shopAddress, setShopAddress] = useState('');
  const [category, setCategory] = useState('');
  
  // 2. New states for handling the Custom input
  ...
  const handleCreateBusiness = () => {
    ...
    setBusinessProfile(prev => ({
      ...prev,
      name: shopName || prev.name,
      address: shopAddress || prev.address,
      category: category || prev.category
    }));
    navigation.goBack();
  };

  // 3. Logic to handle selecting "Custom" vs a standard category
  const selectCategory = (cat) => {
    if (cat === 'Custom') {
      setIsCustomCategory(true);
      setCategory(''); // Clear the text so they can type
    } else {
    ...
    <SafeAreaView style={s.safe} edges={['top']}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={s.container}>
        ...
        <View style={s.header}>
          ...
          <Text style={s.headerTitle}>New Business</Text>
          <View style={{ width: 28 }} />
        </View>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scrollContent}>
          ...
          <View style={s.form}>
            ...
            <View style={s.inputGroup}>
              ...
              {isCustomCategory ? (
                // IF CUSTOM: Show a typing input with an "X" to cancel
          ...
          <TouchableOpacity style={s.createBtn} onPress={handleCreateBusiness} activeOpacity={0.85}>
...
const s = StyleSheet.create({
  ...
  // Standard text input (Shop Name, Address)
  ...
  // Input wrapper for the Custom Text Box
